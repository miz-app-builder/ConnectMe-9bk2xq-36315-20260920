...
-- Idempotent: safe to re-run on a database where some policies already exist.
...
returns boolean language sql security definer stable set search_path = public as $$
  select exists (select 1 from public.conversation_participants cp
    where cp.conversation_id = p_conversation_id and cp.user_id = p_user_id and cp.role = 'admin');
$$;
...
drop policy if exists "conversation members update" on public.conversations;
create policy "conversation members update" on public.conversations for update
using (public.is_conversation_member(id) and (not is_group or public.is_conversation_admin(id)))
with check (public.is_conversation_member(id) and (not is_group or public.is_conversation_admin(id)));
...
create policy "conversation members read participants" on public.conversation_participants for select
using (public.is_conversation_member(conversation_id));

drop policy if exists "conversation participants add" on public.conversation_participants;
drop policy if exists "conversation admins or creator add participants" on public.conversation_participants;
create policy "conversation admins or creator add participants" on public.conversation_participants for insert
with check (public.is_conversation_admin(conversation_id) or exists (
...
drop policy if exists "conversation admins or self remove participants" on public.conversation_participants;
create policy "conversation admins or self remove participants" on public.conversation_participants for delete
...
drop policy if exists "conversation admins update roles" on public.conversation_participants;
create policy "conversation admins update roles" on public.conversation_participants for update
...
begin
  if old.sender_id <> new.sender_id or old.conversation_id <> new.conversation_id or old.created_at <> new.created_at then
...
create trigger validate_message_update before update on public.messages
for each row execute function public.validate_message_update();

-- Reactions / pins / typing
...
with check (auth.uid() = user_id and exists (select 1 from public.messages m where m.id = message_reactions.message_id
  and public.is_conversation_member(m.conversation_id)));
...
create policy "pins admins insert" on public.pinned_messages for insert
with check (auth.uid() = pinned_by and public.is_conversation_admin(conversation_id));

drop policy if exists "pins own delete" on public.pinned_messages;
create policy "pins admins delete" on public.pinned_messages for delete
using (public.is_conversation_admin(conversation_id));
...
begin
  ...
  if auth.uid() = old.callee_id then
    if new.offer is distinct from old.offer then raise exception 'callee cannot change call offer'; end if;
    if new.status not in ('ringing','accepted','rejected','ended') then raise exception 'invalid callee call status'; end if;
  elsif auth.uid() = old.caller_id then
    if new.answer is distinct from old.answer then raise exception 'caller cannot change call answer'; end if;
    if old.status = 'ringing' and new.status not in ('ringing','ended','missed','accepted') then raise exception 'invalid caller call status'; end if;
  end if;
  return new;
end;
$$;
...
with check (auth.uid() = sender_id and exists (
  select 1 from public.calls c where c.id = call_signals.call_id and
...
create policy "messages sender delete" on public.messages for update
...
create policy "profile self update" on public.user_profiles for update
